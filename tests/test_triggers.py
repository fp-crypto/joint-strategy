...
def test_harvest_trigger_within_period(
    vaultA, vaultB, providerA, providerB, tokenA, tokenB, joint, user, gov, amountA, amountB
):
    ...
    joint.setMinTimeToMaturity(joint.period() * 0.98, {'from': gov}) # to be able to mine less blocks
...
def test_harvest_trigger_below_range(
        vaultA, vaultB, providerA, providerB, joint, user, gov, tokenA_whale, mock_chainlink, amountA):
    # deposit to the vault
    actions.user_deposit(user, vaultA, tokenA, amountA)
    actions.user_deposit(user, vaultB, tokenB, amountB)
    ...
    actions.swap(joint.tokenA(), joint.tokenB(), amountA*20, tokenA_whale, joint, mock_chainlink)
...
def test_harvest_trigger_above_range(
    vaultA, vaultB, providerA, providerB, joint, user, gov, tokenB_whale, mock_chainlink, amountB):
    # deposit to the vault
    actions.user_deposit(user, vaultA, tokenA, amountA)
    actions.user_deposit(user, vaultB, tokenB, amountB)
    ...
    actions.swap(joint.tokenB(), joint.tokenA(), amountB*20, tokenB_whale, joint, mock_chainlink)
